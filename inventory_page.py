...
def tab_inventory_count(user):
    st.subheader("🧮 الجرد داخل المستودع (Inventory Count)")
    st.caption("قريب من Odoo: جلسة جرد Draft → إدخال المعدود → ترحيل Post + حركة count")
    ...
    with col1:
        scope = st.selectbox(
            ...
            format_func=lambda x: {"materials":"مواد خام", "products":"منتجات", "both":"مواد + منتجات"}[x],
            key="ic_scope"
        )
        note = st.text_input("ملاحظة (اختياري)", key="ic_note", placeholder="مثال: جرد نهاية اليوم...")

    with col2:
        if st.button("➕ بدء جرد جديد", use_container_width=True, key="ic_new_btn"):
            cid = _new_count_header(scope, user, note)
            st.success(f"تم إنشاء جرد جديد ✅ ({cid})")
            st.session_state["ic_selected"] = cid
            st.rerun()

    st.markdown("### 📋 الجردات الأخيرة")
    counts = _list_recent_counts(limit=20)
    if not counts:
        st.info("لا يوجد جردات بعد.")
        return
    ...
    def label(cid):
        ...
        if not c:
            return cid
        return f"{cid} | {c.get('status','')} | {c.get('scope','')} | {c.get('created_at','')}"

    selected = st.selectbox("اختر جلسة جرد", options=options, format_func=label, key="ic_select")
    if not selected:
        st.info("اختر جلسة جرد لمتابعتها.")
        return

    selected_header = next((c for c in counts if c["id"] == selected), {})
    st.write(f"**الحالة:** {selected_header.get('status','')}")
    st.write(f"**النطاق:** {selected_header.get('scope','')}")
    if selected_header.get("note"):
        st.write(f"**ملاحظة:** {selected_header.get('note','')}")

    if selected_header.get("status") != "draft":
        st.warning("هذا الجرد مُرحّل (posted) ولا يمكن تعديله.")
        st.info("يمكنك إنشاء جرد جديد.")
        return

    st.divider()
    st.markdown("### 1) تحميل الأصناف من النظام (Snapshot)")

    if st.button("📥 تحميل/تحديث الأصناف من النظام", use_container_width=True, key="ic_load_btn"):
        _upsert_count_lines_from_system(selected, selected_header.get("scope", "materials"))
        db.collection("inventory_counts").document(selected).set({"updated_at": now_iso()}, merge=True)
        st.success("تم تحميل الأصناف ✅")
        st.rerun()

    st.divider()
    st.markdown("### 2) أدخل الكمية المعدودة (counted_qty)")

    lines = _get_count_lines(selected)
    if not lines:
        st.info("لا يوجد خطوط جرد بعد. اضغط تحميل الأصناف أولاً.")
        return
    ...
    for l in lines:
        ...
        if cq is not None and cq != "":
            try:
                delta = float(cq) - float(l["system_qty"])
            except Exception:
                delta = ""
        table_rows.append({
            "doc_id": l["doc_id"],
            ...
            "اسم": l["item_name"],
            "الوحدة": l["unit"],
            "الكمية بالنظام": float(l["system_qty"]),
            "الكمية المعدودة": cq,
            "الفرق": delta if delta != "" else "",
        })
    ...
    edited = st.data_editor(
        ...
        use_container_width=True,
        hide_index=True,
        disabled=["doc_id", "نوع", "اسم", "الوحدة", "الكمية بالنظام", "الفرق"],
        column_config={
            "الكمية بالنظام": st.column_config.NumberColumn("الكمية بالنظام", step=0.1),
            "الكمية المعدودة": st.column_config.NumberColumn("الكمية المعدودة", step=0.1),
    ...
    cA, cB = st.columns(2)
    with cA:
        if st.button("💾 حفظ المعدود (Draft)", use_container_width=True, key="ic_save_btn"):
            to_save = [{"doc_id": r["doc_id"], "counted_qty": r.get("الكمية المعدودة", None)} for r in edited]
    ...
    with cB:
        if st.button("✅ ترحيل الجرد (Post) وتحديث المخزون", use_container_width=True, key="ic_post_btn"):
            try:
                _post_inventory_count(selected, user)
                st.success("تم ترحيل الجرد وتحديث المخزون ✅ وتم تسجيل حركة count")
                st.rerun()
            except Exception as e:
                st.error(f"فشل ترحيل الجرد: {e}")
